...
void OnConnection(struct evconnlistener* listener, evutil_socket_t fd, struct sockaddr* a,
    int slen, void* base);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < MAX_IO_THREAD_COUNT; i++)
    {
        ...
        worker->base = base;
        // ListenServerImpl::OnConnected
        struct bufferevent* bev = bufferevent_socket_new(base, -1, BEV_OPT_CLOSE_ON_FREE);
        // TCPConnImpl::Enable
        bufferevent_setcb(bev, OnMessage, NULL, OnError, worker);
        bufferevent_enable(
            bev, EV_READ | EV_WRITE | EV_PERSIST); // dummy event, never triggered;
    ...
    // NetworkManager::Initialize ���� ���� m_EventBase
    struct event_base* base = event_base_new();
    assert(base != NULL);

    // ListenServerImpl::Listen
    ...
    int slen, void* arg)
{
    ...
    bufferevent_setcb(
        job_worker->job_out_bev, OnJobComplete, NULL, OnJobError, job_worker);
