...
//#define JAYKAY_MULTI_STATEMENTS
...
#ifndef JAYKAY_MULTI_STATEMENTS
...
mysql_statement_backend::execute(int number)
{
    if (justDescribed_ == false)
    {
        ...
        int status = 0;
        do {
            MYSQL_RES* result = mysql_store_result(session_.conn_);
            if (result_ && result)
            {
                /*
                첫 번째 mysql_next_result 이후 more results 가 없을 것이라 가정하고 코딩함.
                만약 이 스코프 안으로 들어올 경우
                \lib\soci\src\core\statement.cpp
                statement_impl::execute
                을 뜯어 고쳐야 됨.
                post_fetch 을 여러번 하는 방향으로.
                */
                throw mysql_soci_error("JAYKAY_MULTI_STATEMENTS error.", -1);
            }
            ...
            if (result)
            {
                // Cache the rows offsets to have random access to the rows later.
                // [mysql_data_seek() is O(n) so we don't want to use it].
                int numrows = static_cast<int>(mysql_num_rows(result));
                resultRowOffsets_.resize(numrows);
                for (int i = 0; i < numrows; i++)
                {
                    resultRowOffsets_[i] = mysql_row_tell(result);
            ...
            if ((status = mysql_next_result(session_.conn_)) > 0)
            {
                //throw mysql_soci_error("Could not execute statement.", -1);
                throw mysql_soci_error(mysql_error(session_.conn_),
                    mysql_errno(session_.conn_));
            }
...
int mysql_statement_backend::next_result(int number)
{
    /* more results? -1 = no, >0 = error, 0 = yes (keep looping) */
    int status = mysql_next_result(session_.conn_);
