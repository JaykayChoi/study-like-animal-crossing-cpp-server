...
void parse_connect_string(const string & connectString,
    ...
    bool *client_multi_statements_p , bool *found_rows_p)
{
    *host_p = false;
    *user_p = false;
    *password_p = false;
    ...
    *client_multi_statements_p = false;
    *found_rows_p = false;
    ...
    while (i != end)
    {
        ...
        else if (par == "client_multi_statements" && !*client_multi_statements_p)
        {
            if (val != "1")
            {
                throw soci_error("\"client_multi_statements\" option may only be set to 1");
            }
            *client_multi_statements_p = true;
        }
        else if (par == "found_rows" && !*found_rows_p)
        {
            if (val != "1")
            {
                throw soci_error("\"found_rows\" option may only be set to 1");
            }
            *found_rows_p = true;
        }
        else
    ...
    connection_parameters const & parameters)
{
    ...
    bool client_multi_statements, found_rows;
    parse_connect_string(parameters.get_connect_string(), &host, &host_p, &user, &user_p,
        ...
        &client_multi_statements, &found_rows);
    ...
    std::string val;
    if (connect_timeout_p || parameters.get_option("connect_timeout", val))
    {
        if (!connect_timeout_p)
        {
            if (!valid_uint(val))
                throw soci_error("mysql_options() Invalid connect timeout");
            char *endp;
            connect_timeout = std::strtoul(val.c_str(), &endp, 10);
        }
    ...
    if (client_multi_statements || parameters.get_option("client_multi_statements", val))
    {
        if (val != "1")
        {
            throw soci_error("\"client_multi_statements\" option may only be set to 1");
    ...
    if (found_rows || parameters.get_option("found_rows", val))
    {
        if (val != "1")
        {
            throw soci_error("\"found_rows\" option may only be set to 1");
